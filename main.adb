...
begin
   ...
   declare
      T : MyStringTokeniser.TokenArray(1..7) := (others => (Start => 1, Length => 0));
      NumTokens : Natural;
   begin
      MyStringTokeniser.Tokenise(Lines.To_String(S),T,NumTokens); -- Calls Tokenise function with 3 arguments: User input string and a positive integer (TokenArray), output a natural number to NumTokens
